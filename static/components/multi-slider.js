...
class RangeSlider extends HTMLElement {
  ...
  generatePointPositions() {
    return this.allProps.values.map((value) => {
      let percentage =
        ((value - this.allProps.min) /
          (this.allProps.max - this.allProps.min)) *
        100;
  ...
  initPoint(index) {
    ...
    point.style.left = `${
      (this.pointPositions[index] / this.container.offsetWidth) * 100
    }%`;
  ...
  draw() {
    this.points.forEach((point, i) => {
      point.style.left = `${
        (this.pointPositions[i] / this.container.offsetWidth) * 100
      }%`;
    });
  ...
  documentMouseMoveHandler(e) {
    ...
    this.allProps.values[this.selectedPointIndex] =
      this.possibleValues[valueIndex];
