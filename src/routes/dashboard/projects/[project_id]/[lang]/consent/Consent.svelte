<script lang="ts">
	...
	async function saveConsent() {
		const { error } = await Default.createConsent({
...
<h2 class="font-bold text-xl m-0">Consent</h2>
<p class="my-4">
...
<form class="flex flex-col gap-4">
	<div class="flex flex-col">
		<label for="consent-title" class="mb-2 mt-4 block">Title</label>
		<input
			id="consent-title"
			bind:value={title}
	...
	<div class="flex flex-col">
		<label for="consent-text" class="mb-2 mt-4 block">Message</label>
		<textarea
			id="consent-text"
			bind:value={text}
...
{#if notification}
	<div
		class="fixed bottom-4 right-4 z-[2000] rounded p-4 shadow-lg"
