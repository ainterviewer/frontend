...
test('draws the seed codebook and opens the inspector on a code', async () => {
	...
	const trust = page.getByText('Trust').first();
	await expect.element(trust).toBeVisible();
	await expect.element(page.getByText('Getting in the door').first()).toBeVisible();
...
test('renaming a code in the inspector renames it on the canvas', async () => {
	...
	await page.getByText('Language and comprehension').first().click();
	const name = page.getByPlaceholder('Untitled code');
	await expect.element(name).toBeVisible();
	await name.fill('Language barriers');

	await expect.element(page.getByText('Language barriers').first()).toBeVisible();
});
