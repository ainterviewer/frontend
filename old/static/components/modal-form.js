class ModalForm extends HTMLElement {
	...
	setupEventListeners() {
		...
		submitButton.addEventListener('click', () => {
			...
			if (validationResult.isValid) {
				...
				errorMessage.textContent = validationResult.message || 'Please fill all required fields';
	...
	collectFormData() {
		...
		const radioGroups = {};
		const radioButtons = this.querySelectorAll('[slot="modal-form-group"] input[type="radio"]');
		...
		// Process checkboxes
		const checkboxes = this.querySelectorAll('[slot="modal-form-group"] input[type="checkbox"]');
	...
	validateForm() {
		...
		const fieldsets = this.querySelectorAll('[slot="modal-form-group"] fieldset[required]');
		fieldsets.forEach((fieldset) => {
			const radioButtons = fieldset.querySelectorAll('input[type="radio"]');
			const radioGroupName = radioButtons.length > 0 ? radioButtons[0].name : null;
		...
		if (this.id.startsWith('delete')) {
			const projectName = this.querySelector("label[for$='-name'] > span").textContent;
	...
	resetForm() {
		...
		const fieldsets = this.querySelectorAll('[slot="modal-form-group"] fieldset');
		fieldsets.forEach((fieldset) => {
			fieldset.classList.remove('error');
		});
	}
